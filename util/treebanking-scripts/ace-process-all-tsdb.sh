...
for profile in "$directory"/*; do
  echo $profile
  delphin process --options="-y --yy-rules --max-chart-megabytes=24000 --max-unpack-megabytes=36000 --timeout=20" -g ~/delphin/SRG/grammar/srg/ace/srg.dat --select i-tokens "$profile"
done
